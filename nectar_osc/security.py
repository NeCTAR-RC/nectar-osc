...
class LockInstance(SecurityCommand):
    ...
    def take_action(self, parsed_args):
        ...
        fd_conf = freshdesk.get_config(api_key=conf.freshdesk.api_key,
                                       email_config_id=conf.freshdesk.email_config_id,
                                       group_id=conf.freshdesk.group_id,
                                       domain=conf.freshdesk.domain)

        fd = freshdesk.get_client(fd_conf['domain'], fd_conf['api_key'])

        if parsed_args.no_dry_run is None:
        ...
        if ticket_url:
            ...
            if parsed_args.no_dry_run is None:
                ...
                ticket = fd.tickets.create_outbound_email(
                    ...
                    email=email,
                    cc_emails=cc_emails,
                    email_config_id=int(fd_conf['email_config_id']),
                    group_id=int(fd_conf['group_id']),
                    priority=4,
                    status=2,
                    tags=['security'])
                ticket_id = ticket.id
...
class UnlockInstance(SecurityCommand):
    ...
    def take_action(self, parsed_args):
        ...
        if ticket_url:
            ...
            # if parsed_args.no_dry_run is not None:
